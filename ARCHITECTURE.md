...
| `gandalf/wizard.py` | Declarative builder — `Wizard` (fluent `.step()` / `.branch()` API), a value carrying its shape and nothing else, and `ConfiguredWizard`, what `run.wizard` is: the declared tree with a `form_view` on every step, plus the seams the viewset built it with (`template_name`, `file_storage_class`, `observer_class`, `cursor_walker_class`, `step_dispatcher_class`, `state_serializer_class`, `step_router_class`, `form_view_factory`). Every seam is a `WizardViewSet` class attribute — a wizard is the `Form` and the viewset the `FormView` — and `configure_wizard()` builds one `ConfiguredWizard` per declaration per viewset class and keeps it, so a static `wizard` has one identity across requests |
...
graph LR
    ...
    subgraph Declaration layer
        CW["ConfiguredWizard\n(built once per WizardViewSet class\nfrom its attributes)"]
        S1["tree.Step\n(AccountTypeForm)"]
        BR["tree.Branch\n(arms + default)"]
        S2a["tree.Step\n(BusinessDetailsForm)"]
        S2b["tree.Step\n(PersonalDetailsForm)"]
...
state is reachable more ways than it looks: a re-opened stash, a step that
...
rows, re-openings, links in email — and names a step instead: the cursor's,
...
A **stash payload** (`Run.stash()`) reuses this exact entry shape: `{"version": 1, "label"?: ..., "state": [...], "meta"?: {...}}` where `state` is the stored list with every `files` key stripped (uploads do not outlive completion) and `meta` is the run's bag, which does ride along — a file ref names bytes completion deletes, a record id names something that outlives the run entirely. Re-opening (`reopen()`) is just `initialise_run()` + `set_state()` with a deep copy of that list (plus `set_run_metadata()` when the payload carries a bag) — the lockstep walk then re-proves every answer, so a stash is trusted no further than a live run's own state, and the positional-alignment caveats above apply to stashes across deploys just as they do to live sessions.
...
The third is the one that makes a user-supplied step `FormView` safe. `.form` reconstruction and step dispatch drive the view's composition hooks (`get_form_class()`, `get_form_kwargs()`, `get_initial()`, `get_prefix()`), so a view that reads `request.run` there is reading *from inside* a walk. Without the handoff that read starts a nested walk, which dispatches the same view, which reads again — unbounded recursion. With it, the read sees the prefix already validated on this walk.
...
An address reads as one row wherever it is asked, so `summary_rows` lives
...
**Gandalf ships no templates, and renders only a value.** Not one shipped —
every template on the page belongs to the application or to a design system
built over it. And what the library *renders* is one row's value, through a
template the caller named: never a region, never the page. The distinction
is the whole of the boundary. Once a library renders a region, every piece
of furniture in it — a warning between two rows, a heading, a link — has to
enter through its API, and the API grows an object per kind of furniture,
which is the shape django-crispy-forms has. Gandalf never owns a page
render, so a warning between rows 2 and 3 goes in the template, where it
can be seen.

Both halves are checkable, which is why they replaced the rule they grew
out of — "templates for values, never page structure" — that had a judgment
call in it and eroded within a day. `ls gandalf/templates` returns nothing;
`render_to_string` appears once in the library, in `SummaryMixin.render_row`,
and the name it is given always came from the caller.

The consequence for the page: `row.answer` is display text whichever way it
was made, so a review template is one loop with nothing to decide — no
branch on which spec produced a row, and no name to fall back to. The page
reads back the words the declaration wrote: `Question` and `Answer` there,
...
(`check_row_specs`) called from three places rather than three functions
...
summary row is *one thing the user can change*, not one step. The rows are a
flat list because of it — a step that asked three things is three rows
sharing a change link, and a step that asked one is one row — and a row
therefore has exactly one name, one value and one place to go. A feature
that would give a row several of any of those is a feature about steps
...
**Scope is the store's, identity is the URL's.** Everything a page keeps sits under one record per journey (`session["gandalf_journeys"][journey]`), so two journeys in one session never see each other. The library does not decide when a journey begins, and neither does a wizard: a journey is an id, a record kept under it and a page to show it on, so beginning one is minting the id. `GrantApplication.begin(request)` hands back a `Journey` with a new id, its store and its page URL, and a plain view that redirects to it is a complete beginning. A setup wizard is for the case where the journey's first fact has to be *asked* for rather than already known — `finish()` then records that wizard's run as a section of the list, so it arrives complete and re-openable — and an application that already knows the fact writes it to `store.metadata` instead and skips the wizard. Nor does beginning need a browser: an id and a record are not HTTP, so `begin_for(context)` is the same thing for a management command or an agent, and only `finish()` builds a request, because recording a section dispatches a Django view. A page mounted under `<journey>` reads the id off `self.kwargs` and, since every entry is mounted beneath it and built with the same `(journey, journey_url_kwarg)`, every entry reads the same one. A page mounted under nothing declares `journey = "default"` — one per session, with nothing to begin at all. `begin()` is asked of the `TaskList` value, not the view: a journey is a record in a store, and anything may begin one; the page only shows and ends one.

**The memory is the add-another bargain generalised.** An add-another page caches an item's title at completion so a row never walks; `store.metadata` does the same for any fact the journey turns on. A stash is positional against an unwalked tree, so "did they say they are employed?" costs a walk to read out of one — and `blocked()` and `hidden()` run inside the row's no-walk promise. So the section's `done()` pays the walk once, while the run is still readable, and writes the decided fact; every later render reads a string. `JourneyMetadata` is `MetadataBag` with a `journey` bucket and a `members` bucket, the same two-bucket shape `RunMetadata` has for the same reason.
