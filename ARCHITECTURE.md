...
A hub's sections add up to something — an application, a claim — and that something needs three things a hub does not have: a scope, a memory and an ending. All three live on the section store, which is why `SessionSectionStore` is built with a `journey` as well as a context.

**Scope is the store's, identity is the URL's.** Everything a hub keeps sits under one record per journey (`session["gandalf_journeys"][journey]`), so two journeys in one session never see each other. The library does not mint journeys: the first wizard's `done()` does, and puts the id in the URL. A hub mounted under `<journey>` reads it off `self.kwargs`, hands it to every section view it runs, and every section mounted under the same segment reads the same one. A hub mounted under nothing declares `journey = "default"` — one per session — and `_validate_sections()` refuses a section viewset whose `(journey, journey_url_kwarg)` differ from the hub's, because it would finish into a record the hub never reads.

**The memory is the collection's bargain generalised.** A collection caches an item's title at completion so a row never walks; `store.data` does the same for any fact the journey turns on. A stash is positional against an unwalked tree, so "did they say they are employed?" costs a walk to read out of one — and `blocked()` and `hidden()` run inside the row's no-walk promise. So `section_done()` pays the walk once, while the run is still readable, and writes the decided fact; every later render reads a string. `JourneyData` is `MetadataBag` with a `journey` bucket and a `sections` bucket, the same two-bucket shape `RunMetadata` has for the same reason.

**Hidden is gone; blocked is waiting.** `hidden()` is applied in `_vetted_sections()` after validation, so a hidden section is absent from the rows, the counts and the door — a stale link hits `SectionNotFound` like any undeclared key. `blocked()` keeps the row, locks it, and holds the hub off `COMPLETE`. Hidden outranks blocked by construction: a section that is not there is never asked.

**The ending mirrors `done()`.** `submit()` refuses an incomplete hub, runs `journey_done()` inside the window where the stashes are still readable, and only then `store.complete()` — the runs and stashes go, `data` stays, a tombstone remains — so a `journey_done()` that raises leaves every section resumable. After that, `HubMixin.dispatch()` answers the page and the door with `journey_completed()`, and `SectionMixin.dispatch()` sends a bookmarked step URL back to the hub: a submitted journey has no way back in. `SectionStore` in `gandalf/types.py` names the whole contract, so a durable store — `tests/testapp/durable.py` is one, scoped by owner and journey — drops in by `section_store_class` alone.
