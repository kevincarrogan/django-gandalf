...
| `gandalf/wizard.py` | Declarative builder — `Wizard` (fluent `.step()` / `.branch()` API) and `ConfiguredWizard` (post-`.configure()`, holds the configured tree and pluggable class slots: `file_storage_class` (plus `file_storage_backend`, the Django `Storage` it is handed — passed only when configured, so a zero-argument storage class stays constructible), `cursor_walker_class`, `step_dispatcher_class`, `state_serializer_class`, `step_router_class`, `form_view_factory`). Storage is deliberately not among them — it lives on `WizardViewSet`, since it must exist before `get_wizard()` can be called |
...
A **stash payload** (`BoundWizard.stash()`) reuses this exact entry shape: `{"version": 1, "label"?: ..., "state": [...]}` where `state` is the stored list with every `files` key stripped (uploads do not outlive completion). Resurrection (`resurrect()`) is just `initialise_run()` + `set_state()` with a deep copy of that list — the lockstep walk then re-proves every answer, so a stash is trusted no further than a live run's own state, and the positional-alignment caveats above apply to stashes across deploys just as they do to live sessions.
