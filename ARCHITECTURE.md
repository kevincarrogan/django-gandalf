...
| `gandalf/hubs.py` | Hub and spoke — `Hub()` is the declaration: an immutable builder (`.member()`, `.collection()`, `.hub()`, `.link()`, `.configure()`) naming a task list's members in order, each with its wizard, its title, the step it re-opens at, its `done`, and its `blocked` / `hidden` rules. `HubViewSet` is the page, the door and the mount: set `hub` and `url_name`, and at class creation `materialise()` builds one `MemberViewSet` subclass per wizard member (carrying the wizard, the full key, the hub's URL name and both stores), one `CollectionViewSet` per collection, and one subclass of the root itself per nested hub — so a hook overridden on the root applies to the whole tree — and `urls()` mounts all of them beneath the page, door last. What used to be declared twice and checked for drift is declared once. The page puts a `HubPage` in the template context: one `MemberRow` per member, each carrying a title, a status (`NOT_STARTED` / `INCOMPLETE` / `COMPLETE` / `BLOCKED`) and the hub's own door URL, plus the counts and the derived status of the set. Rows are built once per request. Status is derived from the *shape* of stored state and the presence of a stash, so a row costs storage reads and never a walk. Whether a member is open yet, and whether it is listed at all, are the member's `blocked` and `hidden` rules — one read of the journey's store each, sitting on the row they gate — consulted through `member_blocked()` / `member_hidden()`, the hub's hooks for a rule spanning rows: blocked makes a row read **Cannot start yet** *and* makes the door refuse it; hidden removes the member from `_vetted_members()` so it is absent from the rows, the counts and the door alike. The hub is scoped to a *journey* (`get_journey()`: a URL kwarg when mounted under one, else the declared `journey`), and because every member is mounted beneath it the page's own URL kwargs reach every member URL (`member_url_kwargs()`). It owns an ending: `submit()` refuses an incomplete hub, then a nested hub runs `hub_done()` and returns to its parent while a root runs `journey_done()`, then `store.complete()`; `dispatch()` answers a tombstoned journey with `journey_completed()` at the root and a redirect up everywhere beneath it. The door (`enter()`) is where the one unavoidable walk happens, for the member the user clicked: it resumes a live run, re-opens a stash, or starts a fresh one, and every arm ends at `BoundWizard.entry_url()` — and since a wizard member's bare URL *is* the door, no link can ever be a bare run URL. `MemberViewSet.done()` stashes the finished answers, calls `run_recorded()` (the library's own bookkeeping, inside the window where the run is still readable), then the declared `done`, then clears the run. A `.link()` member has no viewset: given `url_name` and `status`, its row links past the door to something the hub does not run |
| `gandalf/collections.py` | Add another — `Collection(wizard, ...)` is the declaration (item name, what titles an item, `min_items`, the step an item re-opens at, its `done`, its two templates), listed on a `Hub` with `.collection()` or set on a root `CollectionViewSet`. A collection is a hub whose members are *built* from `SessionCollectionStore`'s registry rather than declared, so `CollectionViewSet` extends `HubViewSet` and inherits the status derivation, the row building and the resume-before-reopen door unchanged; at class creation it builds one `ItemViewSet` subclass from the declaration's wizard and mounts it beneath the item door. What is its own: an item's key is composed per request from a URL kwarg (keyed `<collection>:<item>`), its title is cached at completion rather than derived at render, completeness is the user's declared answer to *add another* rather than anything storage can infer — which is why a collection page suppresses the hub's own context object and publishes only `collection`, rather than carry two differently-derived statuses for one page — and the page owns four actions instead of one door — add, change, remove, declare. `CollectionPage` *is* a `HubPage` — same rows, same status, same counts — plus what a hub has no notion of: where the page is, whether the user has declared there are no more, and `min_items`. `urls()` publishes the page (GET lists, POST answers), the item door (GET only, since the route that destroys an item is the other one), the remove route, and the item wizard's run routes beneath the door |
...
**Three routes, and the item wizard beneath them.** The collection publishes the page, `<uuid:item>/` and `<uuid:item>/remove/`, then the item wizard's run routes under `<uuid:item>/`. The item kwarg is a uuid rather than a slug precisely so `remove/` can be a sibling of the door. The item wizard's bare start URL — the one that would complete a valid run on a GET — is deliberately not published: the door occupies that path, so the collection owns every URL an item has. The same shape holds one level up: a hub mounts each member under its own segment and publishes its `<slug:member>/` door *last*, so a collection's or a nested hub's segment is reached directly and a wizard member's segment is the door under the wizard's own URL name.

---

## A journey: what the members add up to

A hub's members add up to something — an application, a claim — and that something needs three things no single wizard has: a scope, a memory and an ending. Every member has the first two through its journey store, and the *root* hub — the one no hub lists — owns the third, which is why `SessionJourneyStore` is built with a `journey` as well as a context.

**Scope is the store's, identity is the URL's.** Everything a hub keeps sits under one record per journey (`session["gandalf_journeys"][journey]`), so two journeys in one session never see each other. The library does not mint journeys: the first wizard's `done()` does, and puts the id in the URL. A hub mounted under `<journey>` reads it off `self.kwargs` and, since every member is mounted beneath it and built with the same `(journey, journey_url_kwarg)`, every member reads the same one. A hub mounted under nothing declares `journey = "default"` — one per session.

**The memory is the collection's bargain generalised.** A collection caches an item's title at completion so a row never walks; `store.data` does the same for any fact the journey turns on. A stash is positional against an unwalked tree, so "did they say they are employed?" costs a walk to read out of one — and `blocked()` and `hidden()` run inside the row's no-walk promise. So the member's `done` pays the walk once, while the run is still readable, and writes the decided fact; every later render reads a string. `JourneyData` is `MetadataBag` with a `journey` bucket and a `members` bucket, the same two-bucket shape `RunMetadata` has for the same reason.

**Hidden is gone; blocked is waiting.** A `hidden` rule is applied in `_vetted_members()`, so a hidden member is absent from the rows, the counts and the door — a stale link hits `MemberNotFound` like any undeclared key. A `blocked` rule keeps the row, locks it, and holds the hub off `COMPLETE`. Hidden outranks blocked by construction: a member that is not there is never asked. Both rules are handed the store and nothing else — the row's no-walk promise, made explicit in the signature.

**The ending mirrors `done()`.** `submit()` refuses an incomplete hub, runs `journey_done()` inside the window where the stashes are still readable, and only then `store.complete()` — the runs and stashes go, `data` stays, a tombstone remains — so a `journey_done()` that raises leaves every member resumable. After that, `JourneyScoped.dispatch()` answers the root's page and doors with `journey_completed()`, and sends a nested hub or a bookmarked step URL back up: a submitted journey has no way back in. `JourneyStore` in `gandalf/types.py` names the whole contract, so a durable store — `tests/testapp/durable.py` is one, scoped by owner and journey — drops in by `journey_store_class` alone.

**A hub is a member, so hubs nest.** `JourneyScoped` is what a `MemberViewSet` and a `HubViewSet` share — `member_key`, `hub_url_name`, `journey` / `journey_url_kwarg`, `journey_store_class`, and the tombstone guard in `dispatch()`. Nesting is a key namespace over the one journey record, not a second store: a nested `Hub` is materialised as a subclass of the root viewset with `member_key` set to its key under the parent's prefix, and `full_key(member)` prefixes it onto every member it lists (`"supporting:referees"`), which is the composition a collection was already doing for its items (`"budget:<uuid>"`) — an item's `Member.key` is just its id. Every store access in the hub goes through `full_key()`, and every generated viewset is handed its full key and its parent's URL name at creation, so there is nothing left to drift and nothing to check. A hub child's row costs `status_for()` — instantiate the child, `setup()` it under the parent's URL kwargs, read `get_hub().status` (a collection answers `get_collection().status`) — so it is the child's rows' storage reads and still no walk; its row on the parent and its segment under the parent both *are* its page. `submit()` branches on `is_nested` (`hub_url_name is not None`): a nested hub runs `hub_done()` — back to the parent by default — and tombstones nothing, because its completion is derived from its rows exactly as the parent reads it; only the root runs `journey_done()` and `store.complete()`, which takes every nested run and stash with it. Being a subclass of the root, a nested hub inherits the root's overrides — wording, titles, `stash_unusable()` — while `journey_done()` / `journey_completed()` are only ever consulted at the root. `get_hub_url()` on every member is the hub *above*; a hub's own page is `get_page_url()`. `JourneyStore` / `CollectionStore` are unchanged by any of this — a durable store needs no knowledge of depth.
