...
The consequence for the page: `row.value` is display text whichever way it
was made, so a review template is one loop with nothing to decide — no
branch on which spec produced a row, and no name to fall back to.
