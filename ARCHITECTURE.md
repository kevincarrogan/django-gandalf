...
A hub's members add up to something — an application, a claim — and that something needs three things a hub does not have: a scope, a memory and an ending. All three live on the member store, which is why `SessionJourneyStore` is built with a `journey` as well as a context.
...
**The memory is the collection's bargain generalised.** A collection caches an item's title at completion so a row never walks; `store.data` does the same for any fact the journey turns on. A stash is positional against an unwalked tree, so "did they say they are employed?" costs a walk to read out of one — and `blocked()` and `hidden()` run inside the row's no-walk promise. So `member_done()` pays the walk once, while the run is still readable, and writes the decided fact; every later render reads a string. `JourneyData` is `MetadataBag` with a `journey` bucket and a `members` bucket, the same two-bucket shape `RunMetadata` has for the same reason.
