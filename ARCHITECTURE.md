...
| `gandalf/hubs.py` | Hub and spoke — `HubMixin` / `HubView` put a `Hub` in the template context: one `MemberRow` per declared `Member`, each carrying a title, a status (`NOT_STARTED` / `INCOMPLETE` / `COMPLETE` / `BLOCKED`) and the hub's own door URL, plus the counts and the derived status of the set, so a task list's heading and its submit button read an object rather than a loop in the view. Rows are built once per request, since both the `Hub` and — on a collection — the `Collection` wrap the same list. Status is derived from the *shape* of stored state and the presence of a stash, so a row costs storage reads and never a walk. Whether a member is open to the user yet, and whether it is listed at all, are the two questions the member answers rather than the hub — `JourneyMemberMixin.blocked()` and `JourneyMemberMixin.hidden()`, classmethods on its own viewset — or on its own hub view, for a member that is a hub — handed the journey's store, so a rule about one member lives with the thing it gates instead of as an arm of a hub method with a key in scope. Both halves consult them through `member_blocked()` / `member_hidden()`, the hub's hooks for what a member cannot answer alone: blocked makes a row read **Cannot start yet** *and* makes the door refuse it, so a stale link cannot start a run the page said was locked; hidden removes the member from `_vetted_members()` — validated first, then filtered — so it is absent from the rows, the counts and the door alike. The hub is scoped to a *journey* (`get_journey()`: a URL kwarg when mounted under one, else the declared `journey`), forwards that kwarg into every member view it runs (`member_url_kwargs()`), and owns an ending: `submit()` refuses an incomplete hub, then a nested hub runs `hub_done()` and returns to its parent while a root runs `journey_done()`, then `store.complete()`; `dispatch()` answers a tombstoned journey with `journey_completed()` on the page and the door alike, and `WizardMemberMixin.dispatch()` does the same for a bookmarked step URL. The door (`enter()`) is where the one unavoidable walk happens, for the member the user clicked: it resumes a live run, re-opens a stash, or starts a fresh one, and every arm ends at `BoundWizard.entry_url()` so no hub link can ever be a bare run URL. `WizardMemberMixin` goes on a member's own viewset and owns `done()`, stashing the finished answers, then calling `run_recorded()` (the library's own bookkeeping hook, inside the window where the run is still readable) before handing off to `run_done()`. A `Member` need not be a wizard: given `url_name` and `status` instead of a `viewset`, its row links past the door to something the hub does not run |
| `gandalf/collections.py` | Add another — a collection is a hub whose members are *built* from `SessionCollectionStore`'s registry rather than declared, so `CollectionMixin` extends `HubMixin` and inherits the status derivation, the row building and the resume-before-reopen door unchanged. What is its own: an item's key is composed per request from a URL kwarg (`ItemMemberMixin`, keyed `<collection>:<item>`), its title is cached at completion rather than derived at render, completeness is the user's declared answer to *add another* rather than anything storage can infer — which is why a collection page suppresses the hub's own context object and publishes only `collection`, rather than carry two differently-derived statuses for one page — and the page owns four actions instead of one door — add, change, remove, declare. `Collection` *is* a `Hub` — same rows, same status, same counts — plus what a hub has no notion of: where the page is, whether the user has declared there are no more, and `min_items`. `CollectionView` publishes three patterns: the page (GET lists, POST answers), the item door (GET only, since the route that destroys an item is the other one), and the remove route |
...
**Three routes, and two collisions to avoid.** The collection publishes the page, `<uuid:item>/` and `<uuid:item>/remove/`. The item kwarg is a uuid rather than a slug precisely so `remove/` can be a sibling — `HubView`'s `<slug:member>/` door can never grow one, which is also why a collection page must not be mounted beneath a hub. And `WizardViewSet` publishes `""` as its start URL, so an item wizard must not be mounted beneath its collection either. Hub, collection and item wizard are siblings.
...
**Scope is the store's, identity is the URL's.** Everything a hub keeps sits under one record per journey (`session["gandalf_journeys"][journey]`), so two journeys in one session never see each other. The library does not mint journeys: the first wizard's `done()` does, and puts the id in the URL. A hub mounted under `<journey>` reads it off `self.kwargs`, hands it to every member view it runs, and every member mounted under the same segment reads the same one. A hub mounted under nothing declares `journey = "default"` — one per session — and `_validate_members()` refuses a member viewset whose `(journey, journey_url_kwarg)` differ from the hub's, because it would finish into a record the hub never reads.

**The memory is the collection's bargain generalised.** A collection caches an item's title at completion so a row never walks; `store.data` does the same for any fact the journey turns on. A stash is positional against an unwalked tree, so "did they say they are employed?" costs a walk to read out of one — and `blocked()` and `hidden()` run inside the row's no-walk promise. So `run_done()` pays the walk once, while the run is still readable, and writes the decided fact; every later render reads a string. `JourneyData` is `MetadataBag` with a `journey` bucket and a `members` bucket, the same two-bucket shape `RunMetadata` has for the same reason.

**Hidden is gone; blocked is waiting.** `hidden()` is applied in `_vetted_members()` after validation, so a hidden member is absent from the rows, the counts and the door — a stale link hits `MemberNotFound` like any undeclared key. `blocked()` keeps the row, locks it, and holds the hub off `COMPLETE`. Hidden outranks blocked by construction: a member that is not there is never asked.

**The ending mirrors `done()`.** `submit()` refuses an incomplete hub, runs `journey_done()` inside the window where the stashes are still readable, and only then `store.complete()` — the runs and stashes go, `data` stays, a tombstone remains — so a `journey_done()` that raises leaves every member resumable. After that, `HubMixin.dispatch()` answers the page and the door with `journey_completed()`, and `WizardMemberMixin.dispatch()` sends a bookmarked step URL back to the hub: a submitted journey has no way back in. `JourneyStore` in `gandalf/types.py` names the whole contract, so a durable store — `tests/testapp/durable.py` is one, scoped by owner and journey — drops in by `journey_store_class` alone.

**A hub is a member, so hubs nest.** `JourneyMemberMixin` is what a run's `WizardMemberMixin` and `HubMixin` share — `member_key`, `hub_url_name`, `journey` / `journey_url_kwarg`, `journey_store_class`, `blocked()` / `hidden()`, and the tombstone guard in `dispatch()` — and a `Member(key, viewset)` accepts either. Nesting is a key namespace over the one journey record, not a second store: a nested hub declares `member_key` and `HubMixin.full_key(member)` prefixes it onto every member it lists (`"supporting:referees"`), which is the composition a collection was already doing for its items (`"budget:<uuid>"`) — an item's `Member.key` is now just its id. Every store access in the hub goes through `full_key()`; a wizard member declares its full key and `_validate_members()` checks it, `hub_url_name` and the journey pair for hub children with the same drift checks it applies to wizard children, and additionally refuses a hub child with no `member_key` or no `url_name`. A hub child's row costs `HubMixin.status_for()` — instantiate the child, `setup()` it under the parent's URL kwargs, read `get_hub().status` (a collection answers `get_collection().status`) — so it is the child's rows' storage reads and still no walk; its row and the parent's door both land on the child's page. `submit()` branches on `is_nested` (`hub_url_name is not None`): a nested hub runs `hub_done()` — back to the parent by default — and tombstones nothing, because its completion is derived from its rows exactly as the parent reads it; only the root runs `journey_done()` and `store.complete()`, which takes every nested run and stash with it. `get_hub_url()` on every member is the hub *above*; a hub's own page is `get_page_url()`. `JourneyStore` / `CollectionStore` are unchanged by any of this — a durable store needs no knowledge of depth.
