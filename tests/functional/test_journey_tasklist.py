"""Chapter 14 as a class body: the same journey, the same claims, driven
over the tree `GrantApplication(TaskList)` mounts.

A port of `test_journey.py` against `ch14_tasklist`.
...
def _start(client, applying_as="individual"):
    ...
    response = client.get(reverse("readme-tasklist-start"), follow=True)
...
def _hub(journey):
    return reverse("readme-tasklist", kwargs={"journey": journey})


def _door(journey, member, hub="readme-tasklist"):
    return reverse(f"{hub}-member", kwargs={"journey": journey, "member": member})


def _supporting(journey):
    return reverse("readme-tasklist-supporting", kwargs={"journey": journey})


def _statuses(client, journey, hub="readme-tasklist"):
    response = client.get(reverse(hub, kwargs={"journey": journey}))
    return {row.key: row.status for row in response.context["hub"].rows}


def _finish(client, journey, member, url_name, steps, hub="readme-tasklist"):
    """Enter a member from its hub and drive it to its end."""
    client.get(_door(journey, member, hub), follow=True)
    key = member if hub == "readme-tasklist" else f"supporting:{member}"
...
def _finish_contact(client, journey):
    return _finish(client, journey, "contact", "readme-tasklist-contact", CONTACT)
...
def _finish_referees(client, journey):
    return _finish(
        ...
        journey,
        "referees",
        "readme-tasklist-supporting-referees",
        [("referee", {"referee_name": "Grace", "referee_email": "grace@example.com"})],
        hub="readme-tasklist-supporting",
    )
...
def _finish_documents(client, journey):
    return _finish(
        ...
        journey,
        "documents",
        "readme-tasklist-supporting-documents",
...
def _finish_budget(client, journey):
    page = reverse("readme-tasklist-budget", kwargs={"journey": journey})
...
def test_a_member_that_does_not_apply_is_not_on_the_page(client):
    ...
    nested = _statuses(client, journey, "readme-tasklist-supporting")
...
def test_a_hidden_members_door_is_refused(client):
    ...
    response = client.get(_door(journey, "documents", "readme-tasklist-supporting"))
...
def test_a_member_waiting_on_another_is_listed_but_cannot_start(client):
    ...
    assert (
        _statuses(client, journey, "readme-tasklist-supporting")["referees"] == BLOCKED
    )
    assertContains(response, "Cannot start yet")
    door = _door(journey, "referees", "readme-tasklist-supporting")
...
def test_a_locked_member_unlocks_when_the_one_it_waits_on_finishes(client):
    ...
    response = client.get(_door(journey, "referees", "readme-tasklist-supporting"))
    assert response.status_code == HTTPStatus.FOUND
    assert f"/readme/tasklist/{journey}/supporting/referees/" in response["Location"]
...
def test_a_nested_hubs_submit_returns_to_the_parent_and_tombstones_nothing(client):
    ...
    assert (
        _statuses(client, journey, "readme-tasklist-supporting")["referees"] == COMPLETE
    )
...
def test_the_budget_is_kept_under_the_journey(client):
    ...
    page = reverse("readme-tasklist-budget", kwargs={"journey": second})
...
def test_a_submitted_journey_refuses_every_way_back_in(client):
    ...
    budget = reverse("readme-tasklist-budget", kwargs={"journey": journey})
    ...
    response = client.get(
        reverse("readme-tasklist-contact", kwargs={"journey": journey})
    )
    assertContains(response, "Application submitted")
    run = "11111111-1111-1111-1111-111111111111"
    response = client.get(
        reverse(
            "readme-tasklist-contact-run", kwargs={"journey": journey, "run_id": run}
        )
    )
    ...
    response = client.get(
        reverse("readme-tasklist-supporting-referees", kwargs={"journey": journey})
    )
    assertRedirects(response, _hub(journey), target_status_code=HTTPStatus.OK)
