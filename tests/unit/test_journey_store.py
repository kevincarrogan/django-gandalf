...
from gandalf.storage import SessionJourneyStore, StashNotFound
...
def test_a_journeys_stashes_are_a_stash_store_kept_in_its_record():
    ...
    context = _Context()
    store = SessionJourneyStore(context, "app-1")
...
def test_journeys_keep_their_own_bookkeeping():
    ...
    second = SessionJourneyStore(context, "app-2")
...
def test_a_journeys_identity_is_a_string_whatever_it_was_given():
    ...
    context = _Context()
    journey = uuid.uuid4()
    SessionJourneyStore(context, journey).set_run("contact", "run-1")
...
def test_a_journey_nobody_has_submitted_is_not_complete():
    assert SessionJourneyStore(_Context(), "default").is_complete() is False
...
def test_only_the_most_recently_completed_journeys_are_kept():
    ...
    never pruned."""
    context = _Context()
    SessionJourneyStore(context, "live").set_run("contact", "run-1")
    for number in range(SessionJourneyStore.max_completed_journeys + 2):
        SessionJourneyStore(context, f"done-{number}").complete()
    ...
    assert f"done-{SessionJourneyStore.max_completed_journeys + 1}" in journeys


def test_completing_a_journey_again_moves_it_to_the_back_of_the_queue():
    context = _Context()
    SessionJourneyStore(context, "a").complete()
    SessionJourneyStore(context, "b").complete()
